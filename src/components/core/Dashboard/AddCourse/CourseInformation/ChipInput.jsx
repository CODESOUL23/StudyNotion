...
export default function ChipInput({
  ...
  useEffect(() => {
    if (editCourse) {
      // console.log(course)
      // Handle both array and string formats for tag
      const tags = course?.tag
      if (Array.isArray(tags)) {
        setChips(tags)
      } else if (typeof tags === 'string' && tags.trim()) {
        // If it's a comma-separated string, split it; otherwise treat as single tag
        setChips(tags.includes(',') ? tags.split(',').map(t => t.trim()) : [tags])
      } else {
        setChips([])
      }
    }
